...
export const MonsterBattlePanel: React.FC<MonsterBattlePanelProps> = ({ globalCount }) => {
  const battle = useMonsterBattle(globalCount);
  ...
  return (
    <div className="w-full max-w-3xl mx-auto mt-6 bg-gray-900 rounded-2xl p-6 border border-gray-800 shadow-xl">
      ...
      {(battle.started || battle.memorialVisible) && (
        <div className="relative w-full aspect-video sm:aspect-[21/9] bg-black rounded-xl overflow-hidden border border-gray-800 flex items-center justify-center mt-4">
          ...
          {battle.imageSrc && !battle.memorialVisible && (
            <div className={`relative w-full h-full flex items-center justify-center p-4 ${isHit ? 'animate-hit-shake' : ''}`}>
              <img src={battle.imageSrc} alt="Monster" className="max-w-full max-h-full object-contain drop-shadow-2xl" />
